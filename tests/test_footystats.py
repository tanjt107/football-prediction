...
async def test_pipeline_matches():
    ...
    try:
        await footystats.etl(
            ...
            insert_sql=read_file("sql/footystats/matches/insert.sql"),
            create_sql=read_file("sql/footystats/matches/create.sql"),
            key="example",
            session=session,
            con=con,
        )
...
async def test_pipeline_seasons():
    ...
    try:
        await footystats.etl(
            "season",
            [1625, 2012, 4759],
            insert_sql=read_file("sql/footystats/season/insert.sql"),
            create_sql=read_file("sql/footystats/season/create.sql"),
            key="example",
            session=session,
            con=con,
        )
...
async def test_pipeline_teams():
    ...
    try:
        await footystats.etl(
            "teams",
            [1625, 2012, 4759],
            insert_sql=read_file("sql/footystats/teams/insert.sql"),
            create_sql=read_file("sql/footystats/teams/create.sql"),
            key="example",
            session=session,
            con=con,
        )
