WITH result AS (
  SELECT
    ...
    COALESCE(positions._6, 0) AS uel,
    COALESCE(positions._16 * 0.5, 0) + COALESCE(positions._17, 0) + COALESCE(positions._18, 0) AS relegation,
    leagues._DATE_UNIX
  ...
  WHERE _LEAGUE = 'Germany Bundesliga'
