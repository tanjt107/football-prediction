WITH result AS (
  SELECT
    ...
    COALESCE(positions._1, 0) + COALESCE(positions._2, 0) + COALESCE(rounds.winner, 0) AS promo,
    COALESCE(positions._22, 0) + COALESCE(positions._23, 0) + COALESCE(positions._24, 0) AS relegation,
    leagues._DATE_UNIX
  ...
  WHERE _LEAGUE = 'England Championship'
